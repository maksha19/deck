                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                        {asks.map((ask, index) => (
                            ...
                            >
                                ...
                                <h4 className="font-semibold text-white text-base md:text-lg mb-1 md:mb-2">{ask.title}</h4>
                                <p className="text-white/70 text-xs md:text-sm leading-relaxed">{ask.purpose}</p>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 md:gap-4">
                        {nextSteps.map((step, index) => (
                            <div key={index} className="flex items-start gap-2 md:gap-3">
                                <span className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-gov-sky/20 flex items-center justify-center text-gov-sky text-[10px] md:text-sm font-medium shrink-0 mt-0.5">
                                    {index + 1}
                                </span>
                                <span className={`text-white/80 text-[11px] md:text-sm leading-relaxed ${index === nextSteps.length - 1 ? 'font-semibold text-gov-gold' : ''}`}>
                                    {step}
                                </span>
                            </div>
                        ))}
